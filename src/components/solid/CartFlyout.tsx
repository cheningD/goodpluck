...
import { createEffect, onMount, Show, createSignal, Suspense, Switch, Match, useTransition } from 'solid-js'
import { formatDate } from '@composables/timeUtils'
import { getDeliverySlots, type Basket } from '@composables/basketUtils'
import {
  basketStore, updateIsValidZip, updateZipCode,
  updateOrders, updateActiveOrder, updateSelectedSlot, isCartOpen
} from '@src/store'
...
const CartFlyout: Component<CartProps> = ({ zipCodes }) => {
  ...
  const [addBasketFromOrdersTab, setAddBasketFromOrdersTab] = createSignal<boolean>(false)
  ...
  const addNewBasket = (): void => {
    ...
    updateActiveOrder({ items: [], deliveryDate: currentBasket().selectedSlot })
  ...
  const validateZipCode = (): void => {
    const isValid: boolean = zipCodes.includes(currentBasket().zipCode)
    updateIsValidZip(isValid)
    if (currentBasket().isValidZip) {
      document.cookie = `gp_zip=${currentBasket().zipCode};max-age=31536000;path=/` // Expires in 1 year
    }
  }
  onMount(() => {
  ...
  return <div class={`${!$isCartOpen() ? 'hidden' : ''} z-50 fixed right-0 bottom-0 h-[calc(100vh_-_40px)] bg-slate-100 w-1/4`}
   >
          ...
          <li onClick={updateTab(0)}>
            ...
            >
               <button
                  ...
                  role="tab"
                >
                  <Show when={currentBasket().selectedSlot === ''}
            ...
            <Switch>
                  ...
                  <Show
                    ...
                    fallback={
                       ...
                       <Show
                        ...
                        fallback={
                          <>
                            <h4 class="max-h-[70vh] min-h-[70vh] text-3xl text-center">Add items to your basket.</h4>
                            <button
                    ...
                    isValid: {currentBasket().isValidZip.toString()}
                      ...
                      <div class="flex flex-col gap-y-3">
                        <input
                          ...
                          id="zip"
                          name="zip"
                          value={currentBasket().zipCode}
                          onInput={(e) => { updateZipCode(e.target.value) }}
              ...
              <Match when={tab() === 1}>
                <div class="flex flex-col content-center">
                  <Show when={addBasketFromOrdersTab()} fallback={
                    <>
                        <ul class="max-h-[70vh] min-h-[70vh] overflow-y-auto">
                          {deliverySlots().map((slot: string) => (
                            <>
                      ...
                      <ul class="max-h-[70vh] min-h-[70vh] overflow-y-auto">
                        {currentBasket().orders.map((basket: Basket) => (
                          <>
                            ...
                            >
                              <span class="flex items-center"><svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 rounded-full p-1 text-white bg-orange-800" viewBox="0 0 24 24"><path fill="currentColor" d="M15 13h1.5v2.82l2.44 1.41l-.75 1.3L15 16.69zm4-5H5v11h4.67c-.43-.91-.67-1.93-.67-3a7 7 0 0 1 7-7c1.07 0 2.09.24 3 .67zM5 21a2 2 0 0 1-2-2V5c0-1.11.89-2 2-2h1V1h2v2h8V1h2v2h1a2 2 0 0 1 2 2v6.1c1.24 1.26 2 2.99 2 4.9a7 7 0 0 1-7 7c-1.91 0-3.64-.76-4.9-2zm11-9.85A4.85 4.85 0 0 0 11.15 16c0 2.68 2.17 4.85 4.85 4.85A4.85 4.85 0 0 0 20.85 16c0-2.68-2.17-4.85-4.85-4.85" /></svg>
                                <strong class="text-xl text-orange-800">Your Next Delivery</strong></span>
                              <h4 class="text-2xl font-bold">{basket.deliveryDate} (No items)</h4>
                              <span>Delivery time: 10:00AM</span>
                            </li>
                          </>
                        ))}
