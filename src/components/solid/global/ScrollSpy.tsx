...
const ScrollSpy: Component<IProps> = ({ categorySlug, categories }) => {
	...
	let categoryHasChild =
		categories.filter((col) => col.parent_id === checkCategory.id).length >
		0
			? categories.filter((col) => col.parent_id === checkCategory.id)[0]
			: null;
	...
	return (
		<>
			<div class="hidden md:flex flex-col items-start px-10 py-4">
				<a
				...
				<Show when={categoryParentsCount > 0}>
					<ul class="sticky top-[114px] flex flex-col gap-y-3">
						{getChildCategories(topLevelCategory.id).map(
							(collection) => (
								<li>
									<a
									...
									<Show
										when={
											...
											getChildCategories(collection.id)
												.length > 0
										}
									>
										<ul class="border-l-4 my-3 border-gray-300 pl-4 top-[114px] flex flex-col gap-y-3">
											{getChildCategories(
												collection.id,
											).map((collection) => (
												<li>
