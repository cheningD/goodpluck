            ...
            >
              ...
              >
                <div class="flex flex-col gap-x-0 mt-5 divide-y divide-dashed divide-gray-200 lg:flex-row lg:items-center lg:justify-end lg:gap-x-7 lg:mt-0 lg:ps-7 lg:divide-y-0 lg:divide-solid dark:divide-gray-700">
                  ...
                  <For each={categories}>
                    {(collection, i) => (
                      <Show when={getSubCategories(collection.id).length == 0}
                       fallback={
                        <div class="hs-dropdown [--strategy:static] md:[--strategy:absolute] [--adaptive:none] md:[--trigger:hover] py-3 md:py-6">
                        <a href={`/market/${collection.slug}`} class="flex items-center w-full text-gray-800 hover:text-gray-600 font-medium dark:text-gray-200 dark:hover:text-gray-500 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                          {collection.name}
                        </a>
                        ...
                        <For each={getSubCategories(collection.id)}>
                          {(subCollection, i) => (
                          <a class="inline-flex gap-x-5 w-full p-4 text-gray-600 rounded-lg hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"  href={`/market/${subCollection.slug}`}>
                           {subCollection.name}
                          </a>
                    )}
                        </For>
