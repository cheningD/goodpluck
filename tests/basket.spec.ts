...
test.describe("Basket Sidebar Tests", () => {
  ...
  test(`Upon successful entry, store the Zipcode in the browser's
     local storage under the key gp_zip.`, async ({ page, browserName }) => {
    test.skip(
      browserName === "chromium",
      "Chromium show toBeVisible as hidden after a long timeout!",
    );
    ...
    const localStorage: any = await page.evaluate(() => {
      const gpZip: string | null = localStorage.getItem("gp_zip");
      if (gpZip) {
        return gpZip;
      }
      return null;
  ...
  test(`Ensure the entered Zipcode persists across browser
   sessions by also saving it in a browser cookie.`, async ({
    page,
    browserName,
  }) => {
    test.skip(
